...
/**
 * Example of how to display all registered user IDs and full names.
 */
public final class ListAllUsers {
...
//		}
		...
		String server = "116.127.223.214:9671";
	    String dbName = "cmappl@cltscm";
	    String user = "scmadm";
...
//		details.setDbConn(args[3]);
	...
	static void listAllUsers(final DimensionsConnection connection) {
		// get the current user ID.
		String me = connection.getObjectFactory().getCurrentUser().getName();
		// list all the users.
		List users = connection.getObjectFactory().getBaseDatabase().getUsers(
				null);
		for (Iterator it = users.iterator(); it.hasNext();) {
			User user = (User) it.next();
			String id = user.getName();
			System.out.println((me.equals(id) ? "* " : "  ") + id + " - "
					+ user.getFullUserName());
		}
	}

	private static void usage() {
		System.err.println("java " + ListAllUsers.class.getName() + " \\");
