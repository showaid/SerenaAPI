...
/**
 * Example to show how to query "database options".
 */
...
//        }
    ...
    static void databaseOptions(final DimensionsConnection connection) {
        DimensionsDatabaseOptions opts = connection.getObjectFactory()
                .getBaseDatabaseAdmin().getDatabaseOptions();
        /* Save the DB option's previous value. */
        int before = opts.getProjectStreamOptions(true);
        /* Change the DB option's value. */
        opts.setProjectStreamOptions(DimensionsDatabaseOptions.DBOPTIONS_PROJECTSONLY);
        opts.save();
        /* 'after' is DimensionsDatabaseOptions.DBOPTIONS_PROJECTSONLY. */
        int after = opts.getProjectStreamOptions(true);
        if (after != DimensionsDatabaseOptions.DBOPTIONS_PROJECTSONLY) {
            throw new ConcurrentModificationException();
        }
        /* Now reset the DB option back to its previous value. */
        opts.setProjectStreamOptions(before);
        opts.save();
    }

    private static void usage() {
        System.err.println("java " + DatabaseOptions.class.getName() + " \\");
