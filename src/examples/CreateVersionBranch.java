...
import com.serena.dmclient.objects.VersionBranchDetails;

/**
 * Log in and create a new version branch, then relate it to the current project
 * as a valid branch.
 */
public final class CreateVersionBranch {
	...
	public static void main(final String[] args) {
		...
		try {
			createVersionBranch(connection);
		} finally {
			// disconnect.
			connection.close();
		}
	...
	static void createVersionBranch(
			final DimensionsConnection connection) {
		// create a new details object.
		VersionBranchDetails branchDetails = new VersionBranchDetails();
		branchDetails.setName("example");
		branchDetails.setDescription("CreateVersionBranch example");

		// get the current VersionBranches collection.
		DimensionsDatabaseAdmin defaultDatabase = connection.getObjectFactory()
				.getBaseDatabaseAdmin();
		VersionBranches branches = defaultDatabase.getBranches();

		// create the version branch.
		try {
			branches.add(branchDetails);
			System.out.println("Succeeded creating the branch.");
		} catch (DimensionsRuntimeException e) {
			System.err.println("Failed to create version branch.");
		...
		// add the new branch as a valid branch for the current project.
		try {
			projObj.addChild(SystemRelationship.OWNED, branches.get("example"));
			System.out.println("Succeeded relating the branch.");
		} catch (DimensionsRuntimeException e) {
			System.err.println("Failed to find/relate the new branch.");
			System.err.println(e.getMessage());
		}
	}

	private static void usage() {
		System.err.println("java " + CreateVersionBranch.class.getName()
				+ " \\");
